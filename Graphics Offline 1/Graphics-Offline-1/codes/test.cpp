...
Point Rotation(Point rotationVector, Point fixedVector, double rotationAngle) {
    ...
    if (_rotationVector.dot(_fixedVector) != 0) cout << "They are not perpendicular.\n";
...
void draw_wheel(double x, double y, double radius) {
    ...
    for (i = 0; i <= lineAmount; i++) {
        wheel[i][0] = Point(x + (radius * cos(i * twicePi / lineAmount)), y + (radius * sin(i * twicePi / lineAmount)),
                            wheelCentre.z);
        wheel[i][1] = Point(x + (radius * cos(i * twicePi / lineAmount)), y + (radius * sin(i * twicePi / lineAmount)),
                            wheelCentre.z + 5);
